  ...
  const categorySpend = useMemo(() => {
    return budgets.map(b => {
      const spent = monthExpenses.filter(k => {
        ...
        return b.kasaCategories.some(kc => cat.includes(kc.toLowerCase()) || desc.includes(kc.toLowerCase()));
  ...
  const uncategorized = useMemo(() => {
    ...
    categorySpend.forEach(b => {
      monthExpenses.forEach(k => {
        const cat = (k.category || '').toLowerCase();
        const desc = (k.description || '').toLowerCase();
        if (b.kasaCategories.some(kc => cat.includes(kc.toLowerCase()) || desc.includes(kc.toLowerCase()))) matchedIds.add(k.id);
      });
    });
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editId ? '✏️ Kategori Düzenle' : '+ Bütçe Kategorisi Ekle'} maxWidth={480}>
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px', gap: 10 }}>
            <div><label style={lbl}>Aylık Limit (₺)</label><input type="number" value={form.monthlyLimit} onChange={e => setForm(f => ({ ...f, monthlyLimit: parseFloat(e.target.value) || 0 }))} style={inp} /></div>
