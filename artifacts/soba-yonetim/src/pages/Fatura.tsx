...
export default function Fatura({ db, save }: Props) {
  ...
  const openNew = (type: 'satis' | 'alis') => {
    setForm({ type, cariId: '', cariName: '', cariTaxNo: '', cariAddress: '', items: [emptyItem()], discount: 0, payment: 'nakit', dueDate: '', note: '', status: 'taslak' });
    setEditId(null);
    setModal(true);
  };

  const openEdit = (inv: Invoice) => {
    setForm({ type: inv.type, cariId: inv.cariId || '', cariName: inv.cariName, cariTaxNo: inv.cariTaxNo || '', cariAddress: inv.cariAddress || '', items: [...inv.items], discount: inv.discount, payment: inv.payment, dueDate: inv.dueDate || '', note: inv.note || '', status: inv.status });
    setEditId(inv.id);
    setModal(true);
  };
                ...
                <tbody>
                  {form.items.map((it, idx) => (
                    <tr key={idx} style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                      ...
                      <td style={{ padding: '6px 8px', width: 70 }}><input type="number" value={it.quantity} min={1} onChange={e => updateItem(idx, 'quantity', parseInt(e.target.value) || 1)} style={{ ...inp, padding: '7px 8px', fontSize: '0.85rem', width: 60 }} /></td>
                      <td style={{ padding: '6px 8px', width: 100 }}><input type="number" value={it.unitPrice} min={0} onChange={e => updateItem(idx, 'unitPrice', parseFloat(e.target.value) || 0)} style={{ ...inp, padding: '7px 8px', fontSize: '0.85rem', width: 90 }} /></td>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: '14px 16px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <input type="number" value={form.discount} min={0} onChange={e => setForm(f => ({ ...f, discount: parseFloat(e.target.value) || 0 }))} style={{ ...inp, width: 100, padding: '5px 8px', fontSize: '0.85rem', textAlign: 'right' }} />
              </div>
